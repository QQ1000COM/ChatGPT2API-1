...
def create_router() -> APIRouter:
    ...
    async def list_image_conversations(summary: bool = Query(False), authorization: str | None = Header(default=None)):
        identity = require_identity(authorization)
        owner = _owner_id(identity)
        items = (
            conversation_service.list_conversation_summaries(owner)
            if summary
            else conversation_service.list_conversations(owner)
        )
        return {"items": items}
    ...
    async def get_image_conversation(conversation_id: str, authorization: str | None = Header(default=None)):
        identity = require_identity(authorization)
        item = conversation_service.get_conversation(_owner_id(identity), conversation_id)
        if item is None:
