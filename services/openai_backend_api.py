...
class OpenAIBackendAPI:
    ...
    def search(
        ...
        if not prompt:
            raise RuntimeError("search prompt is required")
    ...
    def _wait_search_result(self, conversation_id: str, timeout_secs: float, poll_interval_secs: float) -> Dict[str, Any]:
        ...
        retryable_status = {"status=404", "status=409", "status=423", "status=429", "status=500", "status=502", "status=503", "status=504"}
        while time.time() < deadline:
            try:
                conversation = self._get_search_conversation(conversation_id)
                last_result = self._extract_search_result(conversation_id, conversation)
            except Exception as exc:
                message = str(exc)
                if not any(marker in message for marker in retryable_status):
                    raise
    ...
    def _extract_search_result(self, conversation_id: str, conversation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for url in SEARCH_URL_RE.findall(answer):
            cleaned_url = self._clean_search_url(url)
            if cleaned_url and all(item["url"] != cleaned_url for item in sources):
                sources.append({"title": "", "url": cleaned_url, "snippet": "", "source_type": ""})
        return {
            "conversation_id": conversation_id,
